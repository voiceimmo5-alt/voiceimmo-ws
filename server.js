...
// ─── Endpoints HTTP ──────────────────────────────────────────────────────────
app.get('/',       (req, res) => res.json({ status: 'ok', version: 'v70.14-notes-preserve', service: 'VoiceImmo WS', build: '20260808.1530' }));
...
app.get('/debug', async (req, res) => {
  ...
  gmailOk = true; // Resend
  res.json({ version: 'v70.14-notes-preserve', hasOAI: !!OPENAI_API_KEY, oaiOk, gmailOk, configs: Object.keys(CONFIGS) });
});

app.get('/logs', (req, res) => {
  const n     = parseInt(req.query.n    || '50');
  const since = parseInt(req.query.since|| '0');
  res.json({ logs: LOG_BUFFER.filter(l => l.ts > since).slice(-n), serverTime: Date.now(), version: 'v70.14-notes-preserve' });
});
...
app.get('/stats', async (req, res) => {
  ...
  try { const r = await fetch('https://api.openai.com/v1/models', { headers: { Authorization: `Bearer ${OPENAI_API_KEY}` } }); oaiOk = r.ok; } catch(_) {}
  gmailOk = true; // Resend
  res.json({ ok: true, version: 'v70.14-notes-preserve', uptime: Math.floor(process.uptime()), memory: Math.round(process.memoryUsage().heapUsed/1024/1024), oaiOk, gmailOk, node: process.version, serverTime: Date.now(), activeConnections: wss.clients.size, configs: Object.keys(CONFIGS) });
});
...
const SKELETON_BUILDERS = {
  ...
  CONTROLE_REGLEMENTAIRE: buildPromptControle
};
...
wss.on('connection', (ws, req) => {
  ...
  async function saveLead(leadData, cfgData) {
    try {
      const payload = {
        ...
        notes:            [
          leadData.notes || '',
          leadData.transcript && leadData.transcript.length
  ...
  function connectOAI(callerNum) {
    ...
    oai.on('message', async (data) => {
      ...
      // Accumulation du transcript IA (ancien + nouveau nom d'event OpenAI Realtime)
      if ((m.type === 'response.audio_transcript.delta' || m.type === 'response.output_audio_transcript.delta') && m.delta) curAss += m.delta;
    ...
    await flush();
      ...
      // Source 1 : response.audio_transcript.done (ancien + nouveau nom d'event OpenAI Realtime GA)
      if ((m.type === 'response.audio_transcript.done' || m.type === 'response.output_audio_transcript.done') && curAss) {
